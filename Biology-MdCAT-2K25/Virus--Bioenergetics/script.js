document.addEventListener('DOMContentLoaded', () => {
  ...
  const displayResults = () => {
    const total = currentQuestions.length;
    const percentage = total > 0 ? ((correctAnswers / total) * 100).toFixed(1) : '0';
    ...
    retakeWrongBtn.style.display = wrongQuestionIndices.length > 0 ? 'inline-block' : 'none';
  };
  
  const retakeWrongQuestions = () => {
      const questionsToRetake = wrongQuestionIndices.map(index => currentQuestions[index]);
